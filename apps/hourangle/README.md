...
- **Reducing the risk of loss/damage to a mobile phone outside in the dark.** This includes i) forgetting where it was left, ii) putting it on the car bonnet/hood and driving off later with it still there, iii) it falling off the car's back seat whilst trying to pack-up. The list goes on; its so easy to do in the dark when you are busy or distracted. Any functionality of a smart phone that can usefully be put on something which stays strapped to you in this environment is a good thing, e.g. a smart watch.
- **Being off-grid** Lots of astronomers like to keep it simple and straightforward, The more fancy technology involved the more unreliable the session can become. A smart watch is far less intrusive and distracting than a phone. If it has useful tools you need you can check it and then get back to enjoying the outdoors.
